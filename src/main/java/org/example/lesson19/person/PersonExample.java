...
// 9. Чем equals отличается от сравнения объектов через ==? и отличается ли?
// - оператор == сравнивает ссылки (проверяет, указывают ли переменные на один и тот же объект в памяти)
// - метод equals() сравнивает логическое равенство — т.е., то, что определили в методе (важные поля)

//10. Чем equals и hashcode отличается от сравнение объектов через ==
// - equals() — сравнивает, равны ли два объекта по содержимому (если метод переопределён)
// - hashCode() —  не сравнивает объекты, а генерирует их int-число
